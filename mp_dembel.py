...
def valide_defausse(main, indices):
    ...
    if all(v==valeurs_cartes[0] for v in valeurs_cartes): return True
    if n==3:
...
def tour_joueur(joueur, pioche, derniere_defausse, defausse, premier_tour=False):
    ...
    if joueur["humain"]:
        while True:
            ...
            if len(choix)<1 or len(choix)>4: print("⚠️ 1 à 4 cartes seulement"); continue
            try: indices=[int(c)-1 for c in choix]
            except: print("⚠️ Entrée invalide"); continue
            if any(i<0 or i>=len(joueur["main"]) for i in indices): print("⚠️ Indices hors limite"); continue
            if not valide_defausse(joueur["main"], indices): print("⚠️ Défausse invalide"); continue
            break
    else:
        indices=[0]
        if len(joueur["main"])>=2 and joueur["main"][0].split()[0]==joueur["main"][1].split()[0]: indices=[0,1]

    indices=sorted(indices,reverse=True)
    defausse_courante=[]
    for i in indices: defausse_courante.append(joueur["main"].pop(i))
    print(f"{joueur['nom']} défausse :")
    for c in defausse_courante: print(" -",c)
    defausse.extend(defausse_courante)

    # --- Pioche ---
    if joueur["humain"] and carte_visible_defausse:
        print("\nChoisissez une carte à piocher dans la dernière défausse ou pioche :")
        print("0. Pioche")
        for idx,c in enumerate(carte_visible_defausse,1): print(f"{idx}. {c}")
        while True:
            ...
            try:
                ...
                if idx==0:
                    ...
                    if carte: joueur["main"].append(carte)
                    break
...
def jouer_multijoueur():
    ...
    while True:
        ...
        total=nb_h+nb_ai
        if total>max_joueurs or total<2: print(f"⚠️ Nombre total entre 2 et {max_joueurs}"); continue
        else: break

    joueurs=[]
    for i in range(nb_h): joueurs.append({"nom":f"Joueur {i+1}","main":[],"humain":True})
    for i in range(nb_ai): joueurs.append({"nom":f"Ordi {i+1}","main":[],"humain":False})

    paquet=melanger(creer_paquet())
    for j in joueurs: j["main"]=[piocher(paquet, []) for _ in range(7)]
